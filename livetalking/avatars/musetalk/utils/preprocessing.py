...
try:
    ...
    _HAS_MMPOSE = False
    print(f"[WARN] mmpose unavailable, preprocessing will fallback to face-detector-only bbox: {exc}")

# initialize the mmpose model (optional)
...
def get_bbox_range(img_list,upperbondrange =0):
    ...
    average_range_plus = []
    if not _HAS_MMPOSE:
        return "mmpose not available, bbox_shift auto-range is unavailable in fallback mode."
...
def get_landmark_and_bbox(img_list,upperbondrange =0):
    ...
    average_range_plus = []
    if not _HAS_MMPOSE:
        print("[WARN] Running fallback bbox mode without mmpose landmarks.")
        for fb in tqdm(batches):
            bbox = fa.get_detections_for_batch(np.asarray(fb))
            for f in bbox:
                if f is None:
        ...
        print(f"Total frame:「{len(frames)}」 Fallback mode without landmarks, current bbox_shift: {upperbondrange}")
        print("*************************************************************************************************************************************")
