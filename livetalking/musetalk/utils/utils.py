...
def _require_path(path, kind):
    exists = osp.isdir(path) if kind == "directory" else osp.isfile(path)
    if not exists:
        raise FileNotFoundError(
            f"Missing required MuseTalk {kind}: {path}. "
            f"Download the MuseTalk model bundle into {_project_path('models')} before starting."
        )
    return path


def _resolve_existing_path(paths, kind, label):
    for path in paths:
        exists = osp.isdir(path) if kind == "directory" else osp.isfile(path)
        if exists:
            return path
    raise FileNotFoundError(
        f"Missing required MuseTalk {label}. Checked: {', '.join(paths)}. "
        f"Download the MuseTalk model bundle into {_project_path('models')} before starting."
    )
...
def load_all_model(
    ...
    if unet_model_path is None:
        unet_model_path = _project_path("models", "musetalkV15", "unet.pth")
    if unet_config is None:
        unet_config = _project_path("models", "musetalkV15", "musetalk.json")
    vae_candidates = [_project_path("models", vae_type)]
    if vae_type == "sd-vae-ft-mse":
        vae_candidates.append(_project_path("models", "sd-vae"))
    vae_path = _resolve_existing_path(vae_candidates, "directory", "VAE model directory")

    _require_path(unet_model_path, "file")
    _require_path(unet_config, "file")
