    ...
    def __init__(self, 
        ...
        if not os.path.isdir(model_path):
            raise FileNotFoundError(
                f"Missing MuseTalk Whisper model directory: {model_path}. "
                "Expected a local Hugging Face-style model folder such as models/whisper."
            )
        self.feature_extractor = AutoFeatureExtractor.from_pretrained(model_path, local_files_only=True)
        self.whisper = WhisperModel.from_pretrained(model_path, local_files_only=True)
