...
class ElevenLabsTTS(BaseTTS):
    def __init__(self, opt, parent):
        ...
        self.voice_id = (
            os.getenv("ELEVEN_VOICE")
            or os.getenv("ELEVENLABS_VOICE_ID")
            or opt.REF_FILE
            or ""
        ).strip()
        self.model_id = (os.getenv("ELEVEN_MODEL") or "eleven_turbo_v2_5").strip()
        self.base_url = (os.getenv("ELEVENLABS_BASE_URL") or "https://api.elevenlabs.io").rstrip("/")
        self.output_format = (os.getenv("ELEVEN_OUTPUT_FORMAT") or "pcm_16000").strip()
        self.optimize_streaming_latency = str(
            os.getenv("ELEVEN_OPTIMIZE_STREAMING_LATENCY") or "0"
        ).strip()
        self.request_timeout_sec = float((os.getenv("ELEVEN_REQUEST_TIMEOUT_SEC") or "120").strip())
        self.connect_timeout_sec = float((os.getenv("ELEVEN_CONNECT_TIMEOUT_SEC") or "10").strip())
        ...
        if stability:
            try:
                self.voice_settings["stability"] = float(stability)
            except ValueError:
                logger.warning("Invalid ELEVEN_STABILITY=%s, ignoring.", stability)
        if similarity_boost:
            try:
        ...
        if not self.api_key:
            raise RuntimeError("Missing ELEVENLABS_API_KEY (or ELEVEN_API_KEY) for ElevenLabsTTS.")
        if not self.voice_id:
            raise RuntimeError("Missing ELEVEN_VOICE (or ELEVENLABS_VOICE_ID / --REF_FILE) for ElevenLabsTTS.")
    ...
    def elevenlabs_stream(self, text: str) -> Iterator[bytes]:
        ...
        optimize_latency = (self.optimize_streaming_latency or "").strip()
        # ElevenLabs rejects optimize_streaming_latency for some models (for example eleven_v3).
        if optimize_latency and not self.model_id.lower().startswith("eleven_v3"):
        ...
        try:
            ...
            logger.info("elevenlabs Time to make POST: %ss", round(end - start, 3))
            ...
            for chunk in response.iter_content(chunk_size=4096):
                ...
                if first:
                    ...
                    logger.info("elevenlabs Time to first chunk: %ss", round(end - start, 3))
                    first = False
                yield chunk
        except Exception:
    ...
    def stream_tts(self, audio_stream: Iterator[bytes], msg: tuple[str, dict]):
        ...
        for chunk in audio_stream:
            if not chunk:
                continue
            pcm_bytes = pending_bytes + chunk
            even_bytes_len = len(pcm_bytes) - (len(pcm_bytes) % 2)
            if even_bytes_len <= 0:
