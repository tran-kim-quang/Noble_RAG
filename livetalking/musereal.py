...
def _use_half_precision(device: torch.device) -> bool:
    # FP16 on CPU is extremely slow and can look like startup freeze.
...
def load_model():
    ...
    use_half = _use_half_precision(device)
...
def load_avatar(avatar_id):
    ...
    avatar_candidates = [
        ...
        f"./musetalk/data/avatars/{avatar_id}",
    ]
    def _is_ready_avatar(path):
        return (
            os.path.isdir(path)
            and os.path.isfile(os.path.join(path, "latents.pt"))
            and os.path.isfile(os.path.join(path, "mask_coords.pkl"))
            and os.path.isdir(os.path.join(path, "mask"))
        )

    avatar_path = next(
        (path for path in avatar_candidates if _is_ready_avatar(path)),
        next((path for path in avatar_candidates if os.path.isdir(path)), avatar_candidates[0]),
    )
    ...
    input_latent_list_cycle = (
        ...
        if torch.cuda.is_available()
        else torch.load(latents_out_path, map_location="cpu")
    )  #,weights_only=True
    ...
    # Keep all avatar modalities aligned to avoid index overflow in paste_back_frame.
    usable_len = min(
        len(input_latent_list_cycle),
        len(coord_list_cycle),
        len(frame_list_cycle),
        len(mask_coords_list_cycle),
        len(mask_list_cycle),
    )
    if usable_len <= 0:
        raise RuntimeError(f"Avatar '{avatar_id}' has no usable frames after alignment.")
    if (
        usable_len != len(input_latent_list_cycle)
        or usable_len != len(coord_list_cycle)
        or usable_len != len(frame_list_cycle)
        or usable_len != len(mask_coords_list_cycle)
        or usable_len != len(mask_list_cycle)
    ):
        logger.warning(
            "Avatar '%s' assets length mismatch (latents=%d, coords=%d, full=%d, mask_coords=%d, mask=%d). "
            "Trimming to %d frames.",
            avatar_id,
            len(input_latent_list_cycle),
            len(coord_list_cycle),
            len(frame_list_cycle),
    ...
    input_latent_list_cycle = input_latent_list_cycle[:usable_len]
    coord_list_cycle = coord_list_cycle[:usable_len]
    frame_list_cycle = frame_list_cycle[:usable_len]
    mask_coords_list_cycle = mask_coords_list_cycle[:usable_len]
    mask_list_cycle = mask_list_cycle[:usable_len]
...
def warm_up(batch_size,model):
    ...
    if unet.device.type != "cuda":
        logger.info("Skip warmup on %s to avoid startup stall.", unet.device.type)
        return
    #batch_size = 16
